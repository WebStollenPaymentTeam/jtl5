...
class OrderLine implements JsonSerializable
{
    ...
    public static function factory($oPosition, Currency $currency): self
    {
        ...
        $orderLine->description = $oPosition->cName;
        if (!$orderLine->description || !is_string($orderLine->description)) {
            $orderLine->description = $oPosition->cArtNr ?: '(null)';
        }
        ...
        if (fmod($oPosition->nAnzahl, 1) !== 0.0) {
            $_netto *= $_amount;
            $_amount = 1;
            $orderLine->description .= sprintf(' (%.2f %s)', (float)$oPosition->nAnzahl, $oPosition->cEinheit);
        }
